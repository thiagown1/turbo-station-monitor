...
async function createGroupSuggestion(conversationId, brandId, { sourceMessageId = null } = {}) {
  ...
  if (!conv || conv.channel !== 'whatsapp-group') return null;
  if (conv.status === 'closed' || conv.is_staff) return null;
  ...
  if (!lastMsg || lastMsg.direction === 'outbound') return null;
  ...
  if (!result || !result.text || result.waiting || result.noReply) return null;
  ...
  return { ...result, suggestionId: sugId };
}
...
function scheduleGroupSuggestion(conversationId, brandId, { media = false } = {}) {
  ...
  global._copilotDebounceTimers[conversationId] = setTimeout(async () => {
    delete global._copilotDebounceTimers[conversationId];
    try {
      await createGroupSuggestion(conversationId, brandId);
    } catch (err) {
...
module.exports = { createGroupSuggestion, scheduleGroupSuggestion };
