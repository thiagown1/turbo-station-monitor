...
const BRAND = process.env.TEST_BRAND_ID || process.env.BRAND_ID || 'test_brand';
const fs = require('fs');
const path = require('path');

function workspaceDirForAgent(agentId) {
...
function workspaceDirForBrand(brandId) {
  const agentId = process.env.TEST_AGENT_ID || process.env.OPENCLAW_AGENT || `support_${brandId}`;
  return workspaceDirForAgent(agentId);
}
...
async function main() {
  ...
  const workspaceDir = workspaceDirForBrand(BRAND);
